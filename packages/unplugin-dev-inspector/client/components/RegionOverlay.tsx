...
export const RegionOverlay: React.FC<RegionOverlayProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Annotation Markers */}
      {isAnnotating &&
        identifiedElements.map((item, idx) => (
          <div
            key={idx}
            className={cn(
        ...
        <div
          ...
          style={{
            left: Math.min(
              ...
              Math.max(10, identifiedElements[activeNoteIdx].rect.right + 10),
            ),
            // Ensure it doesn't go off bottom (assuming ~250px height) or top
            top: Math.min(
              window.innerHeight - 250,
              Math.max(10, identifiedElements[activeNoteIdx].rect.top),
            ),
          }}
        ...
        >
          ...
          <div className="text-[10px] text-slate-400 font-mono mb-2 truncate">
            <span className="font-semibold text-slate-600">
              {identifiedElements[activeNoteIdx].info.component || "Element"}
            </span>
            <span className="mx-1.5 opacity-50">•</span>
            {identifiedElements[activeNoteIdx].info.file
              ? `${identifiedElements[activeNoteIdx].info.file.split("/").pop()}:${identifiedElements[activeNoteIdx].info.line}`
              : "Unknown Location"}
          </div>
